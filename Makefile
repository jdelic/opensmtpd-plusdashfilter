...
help:
	@echo ""
	@echo "targets are: filter_plusdash, dist and upload"	
	@echo ""
...
opensmtpd-extras-plusdash_1.0.deb: filter_plusdash
	/usr/local/bin/fpm \
		-t deb \
		-s dir \
		...
		filter_plusdash=/usr/lib/x86_64-linux-gnu/opensmtpd/filter_plusdash
...
upload: opensmtpd-extras-plusdash_1.0.deb
	...
	$(APTLY) publish list -raw | grep -q s3:maurusnet && \
		$(VW) publish update -architectures="amd64" jessie \
			s3:maurusnet:debian || \
